...
select * from rental;
select count(rental_id),month(rental_date) from rental group by month(rental_date);
-- ---------------------------------------------------------------------------------------------------------------------------------------

-- 3. What is the rental rate of the film with the longest title in the database? (2 Marks)
select rental_rate,char_length(trim(title)) as len from film 
...
select avg(rental_rate) from rental r join inventory i using (inventory_id) join film f 
using (film_id) where r.rental_date in (select rental_date from Rental where rental_date 
between "2005-05-05 22:04:30" and (select adddate("2005-05-05 22:04:30" , interval 30 day)) 
...
select f.rental_duration as dur,f.film_id,f.title,i.inventory_id 
from film f left join inventory i using (film_id)
where i.inventory_id is null order by dur desc
limit 1;
-- -----------------------------------------------------------------------------------------------------------------------------------------

-- 7. What is the average rental rate for films, broken down by category? (3 Marks)
select avg(f.rental_rate) as rr, c.name as n from film f join film_category fc 
using (film_id) join category c using (category_id)
group by n;
-- -----------------------------------------------------------------------------------------------------------------------------------------

-- 8. What is the total revenue generated from rentals for each actor in the database? (3 Marks)
select concat(a.first_name,' ',a.last_name) as name,a.actor_id,sum(p.amount)from actor a join film_actor fa using (actor_id)
join film f using (film_id) join inventory i using 
(film_id) join rental r using (inventory_id)
join payment p using (rental_id)
group by a.actor_id;
-- --------------------------------------------------------------------------------------------------------------------------------------

-- 9. Show all the actresses who worked in a film having a "Wrestler" in the description. (3 Marks)
select concat(a.first_name,' ',a.last_name) as name,f.description from actor a 
join film_actor fa using (actor_id) join film f using (film_id) 
where f.description like '%Wrestler%';
-- --------------------------------------------------------------------------------------------------------------------------------------

-- 10. Which customers have rented the same film more than once? (3 Marks)
select c.customer_id,f.film_id,count(*) from film f join inventory i using (film_id)
join rental r using (inventory_id) join customer c using (customer_id) 
group by f.film_id,c.customer_id having count(*)>1;
-- --------------------------------------------------------------------------------------------------------------------------------------

-- 11. How many films in the comedy category have a rental rate higher than the average rental rate? (3 Marks)
select count(f.title)as count_film from film f join film_category fc using (film_id) 
join category c using (category_id) where c.name = 'comedy' and 
...
select f.title,dense_rank() OVER (ORDER BY count(f.title) DESC) AS Ranking,cu.customer_id,
c.city  from customer cu join rental r using (customer_id) join address a 
using (address_id) join city c using (city_id) join inventory i using 
...
join address ad on st.address_id = ad.address_id join city c using (city_id)
join country cy using (country_id) join payment p using (staff_id)
...
join film using (film_id) join customer c using (customer_id)
join payment using (rental_id) group by c.customer_id, rental_date, title, rental_duration, paid_amount;
-- --------------------------------------------------------------------------------------------------------------------------------------------------
